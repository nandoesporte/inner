...
// Using type-only import for Session to handle export issues in some environments
import type { Session } from '@supabase/supabase-js';
import { User, Building, Mail, Shield, Save, Loader2, CreditCard, CheckCircle2, Webhook, Copy, Key, Palette, Image as ImageIcon, Type, Link as LinkIcon, Send } from 'lucide-react';
import { UserRole, TenantSettings } from '../types';
import { candidateImportService } from '../services/candidateImportService';
import AssessmentLinksTab from './AssessmentLinksTab';

interface SettingsViewProps {
  // Using any to bypass strict type check if Session export is missing
  session: any;
  userRole: UserRole;
  tenantId?: string;
...
const SettingsView: React.FC<SettingsViewProps> = ({ session, userRole, tenantId, onSettingsChange, currentSettings }) => {
  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<'profile' | 'billing' | 'api' | 'branding' | 'links'>('profile');
  ...
  // Email Configuration State (Resend)
  ...
  const [brandingSettings, setBrandingSettings] = useState<TenantSettings>({
    ...
    login_title: currentSettings?.login_title || 'Bem-vindo de volta!',
    login_subtitle: currentSettings?.login_subtitle || 'Acesse sua conta para gerenciar avaliações.',
    login_cover_title: currentSettings?.login_cover_title || 'PessoaCerta',
    login_cover_subtitle: currentSettings?.login_cover_subtitle || 'Para todas as empresas que valorizam gente de verdade'
  });
  ...
  useEffect(() => {
    ...
    if (currentSettings) {
        setBrandingSettings(prev => ({ ...prev, ...currentSettings }));
        if (currentSettings.api_token) setApiToken(currentSettings.api_token);
        if (currentSettings.resend_api_key) setResendApiKey(currentSettings.resend_api_key);
        if (currentSettings.mail_from) setMailFrom(currentSettings.mail_from);
        
        // Se não tiver token, gera um se tenantId estiver disponível
        if (!currentSettings.api_token && tenantId && !apiToken) {
            generateAndSaveToken();
        }
    }
  }, [session, tenantId, currentSettings]);
  ...
  const handleSaveEmailSettings = async () => {
      ...
      try {
          ...
          if (error) throw error;
          setSuccessMsg('Configurações de E-mail salvas!');
          setTimeout(() => setSuccessMsg(''), 3000);
      } catch (error: any) {
    ...
    try {
        const { error } = await supabase
            ...
            .upsert({
                ...
                login_title: brandingSettings.login_title,
                login_subtitle: brandingSettings.login_subtitle,
                login_cover_title: brandingSettings.login_cover_title,
                login_cover_subtitle: brandingSettings.login_cover_subtitle,
                api_token: apiToken, 
                // Mantém os valores atuais de email se existirem no estado
                resend_api_key: resendApiKey,
                mail_from: mailFrom,
                updated_at: new Date()
            }, { onConflict: 'tenant_id' });
  ...
  const CopyField = ({ label, value, isSecret }: { label: string, value: string, isSecret?: boolean }) => (
    <div className="space-y-1">
      <label className="text-xs font-bold text-neutral-500 uppercase">{label}</label>
      <div className="flex gap-2">
        <code className={`flex-1 bg-neutral-100 border border-neutral-200 p-2 rounded text-sm text-neutral-700 font-mono truncate ${isSecret ? 'blur-sm hover:blur-none transition-all' : ''}`}>
          {value}
        </code>
        <button 
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex border-b border-neutral-200 overflow-x-auto">
         ...
         <button 
           ...
           className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap flex items-center gap-2 ${activeTab === 'links' ? 'border-brand-blue text-brand-blue' : 'border-transparent text-neutral-500 hover:text-neutral-700'}`}
         >
            <LinkIcon className="w-4 h-4" /> Links de Avaliação
         </button>
         <button 
      ...
      <div className="py-2">
        ...
        {/* TAB: LINKS DE AVALIAÇÃO */}
        {activeTab === 'links' && tenantId && (
            <AssessmentLinksTab tenantId={tenantId} />
        )}
        ...
        {activeTab === 'api' && webhookConfig && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-neutral-100 bg-neutral-50/50">
                    <h3 className="font-semibold text-neutral-800 flex items-center gap-2"><Send className="w-5 h-5 text-brand-blue" /> Provedor de E-mail (Resend)</h3>
                </div>
                <div className="p-8 space-y-6">
                    <p className="text-sm text-neutral-600">
                        Configure suas credenciais do <a href="https://resend.com" target="_blank" rel="noreferrer" className="text-brand-blue hover:underline">Resend.com</a> para enviar convites de avaliação em lote.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-neutral-700">Resend API Key</label>
                            <input 
                                type="password" 
                                value={resendApiKey}
                                onChange={(e) => setResendApiKey(e.target.value)}
                                placeholder="re_123456789..."
                                className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue outline-none font-mono"
                            />
                            <p className="text-xs text-neutral-400">Chave de API com permissão de envio (Sending).</p>
                        </div>
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-neutral-700">Remetente (From)</label>
                            <input 
                                type="email" 
                                value={mailFrom}
                                onChange={(e) => setMailFrom(e.target.value)}
                                placeholder="convites@suaempresa.com"
                                className="w-full px-4 py-2 border border-neutral-200 rounded-lg focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue outline-none"
                            />
                            <p className="text-xs text-neutral-400">Deve ser um domínio verificado no seu painel do Resend.</p>
                        </div>
                    </div>
                    <div className="flex justify-end pt-2">
                         <button 
                            ...
                            className="flex items-center gap-2 px-6 py-2 bg-brand-blue hover:bg-brand-dark text-white rounded-lg font-medium shadow-sm transition-all disabled:opacity-70"
                        >
                            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Salvar Configuração
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
               <div className="px-6 py-4 border-b border-neutral-100 bg-neutral-50/50 flex justify-between items-center">
                 ...
                 <button onClick={generateAndSaveToken} className="text-xs flex items-center gap-1 text-neutral-500 hover:text-brand-blue">
                    <Key className={`w-3 h-3 ${isGeneratingToken ? 'animate-spin' : ''}`} /> Redefinir Token
                 </button>
               </div>
               <div className="p-8 space-y-6">
                  <div className="text-sm text-neutral-600 space-y-2">
                    <p>Envie PDFs automaticamente a partir de outros sistemas (n8n, Zapier). Três métodos suportados:</p>
                    <ul className="list-disc pl-5 space-y-1 mt-2">
                        <li><strong>Recomendado:</strong> POST <code>multipart/form-data</code> com campo <code>file</code>.</li>
                        <li>JSON: <code>{`{"file_url": "https://..."}`}</code></li>
                        <li>JSON: <code>{`{"file_base64": "..."}`}</code></li>
                    </ul>
                  </div>
                  ...
                  <div className="flex items-start gap-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                     ...
                     <div>
                        ...
                        Use este token estático no header <code>x-api-token</code>.
                     </div>
                  </div>
                  {/* Mostra o token persistente (apiToken) ou o mockado se ainda não carregou */}
                  <CopyField label="API Token (Estático)" value={apiToken || webhookConfig.token} isSecret={true} />
               </div>
            </div>
